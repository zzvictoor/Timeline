...
class LegacyTimelineLoader(importlib.machinery.SourceFileLoader):
    def source_to_code(self, data, path, *, _optimize=-1):
        ...
        # lib2to3 expects a trailing newline for reliable parsing.
        if source and not source.endswith("\n"):
            source += "\n"
        try:
            converted = str(_tool().refactor_string(source, path))
        except Exception as exc:
            raise ImportError("Unable to convert legacy Timeline module {}: {}".format(path, exc)) from exc
...
def install_legacy_importer():
    ...
    if _INSTALLED:
        return
    if os.getenv("TIMELINE_LEGACY_IMPORT_COMPAT", "1").lower() in ("0", "false", "no"):
        return
